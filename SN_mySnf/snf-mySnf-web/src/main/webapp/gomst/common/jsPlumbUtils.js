...
function initJsPlumbInstance() {
	...
	INSTANCE_JSPLUMB = jsPlumb.getInstance({
		PaintStyle: {
			strokeStyle: '#2a2929', 
			stroke: '#2a2929',  //color for a Connector
			strokeWidth: 4, //width of a Connector's line
			//fill: 'pink',
			fillStyle: '#1e8151',
			radius: 5
		},
	...
	INSTANCE_JSPLUMB.bind('beforeDrop', function(info) {
		if(info.sourceId !== info.targetId) {
            if(graph.node(getRemovePrefixId(info.targetId)).nodeCode==='SOP'){
                layer.tips(CONFIG.msg.endWithSOPNode, getJquerySelectorPrefix(info.sourceId), {
                    tips: [2, '#23262e'],
                    time: 1000
                });
                return false;
            };
            var sourceNodeCode = graph.node(getRemovePrefixId(info.sourceId)).nodeCode;
            if(sourceNodeCode==='EOP'||sourceNodeCode==='EOPF'){
                layer.tips(CONFIG.msg.startWithEndNode, getJquerySelectorPrefix(info.sourceId), {
                    tips: [2, '#23262e'],
                    time: 1000
                });
                return false;
            };
            if(!checkConnection(info.sourceId)){
                layer.tips(CONFIG.msg.moreThenTwoRouter, getJquerySelectorPrefix(info.sourceId), {
                    tips: [2, '#23262e'],
                    time: 1000
                });
                return false;
	...
	INSTANCE_JSPLUMB.bind('connection', function(a, b) {
		...
		if (obj != undefined) {
			...
			//给连接线添加id、右击菜单
			window.ContextMenu.bind("#" + obj.c, connectionMenuJson); //右击连线
			
			//给连接线添加双击事件
			$('#' + obj.c).dblclick(function(event) {
                connectionAttr("#"+obj.c);
			});
            connectionAttr("#"+obj.c);
		}
	});
...
function addConnectionId2(sourceId, targetId, connId) {
